const httpProxyUrl = process.env.HTTP_PROXY || process.env.http_proxy || process.env.HTTPS_PROXY || process.env.https_proxy;
...
const downloadFiles = async task => {
  ...
  debugger;
  console.log("trying to get " + task.agency_url);
  console.log("with options: " + options);

  const res = await fetch(task.agency_url, options);
  debugger;
  if (res.status !== 200) {
